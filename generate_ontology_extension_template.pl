...
for my $table (get_strings($mart_handle->prepare("show tables like '%_gene__%_extension__dm'"))) {
    ...
    for my $condition (grep {$_ ne 'subject' && $_ ne 'source'} get_strings($mart_handle->prepare("select distinct(column_name) from information_schema.columns where table_name='$table' and table_schema='$mart_db' and column_name like '%_label'"))) {
        $condition =~ s/_label//;
