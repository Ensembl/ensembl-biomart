...
my ( $host, $port, $user, $pass, $dbname, $release, $old_release,
     $tmp_dir );
...
my $sel_sth =
  $dbh->prepare('SELECT dataset_id_key, xml FROM meta_conf__xml__dm;');
$sel_sth->execute();
...
'UPDATE meta_conf__xml__dm SET xml = ?, compressed_xml = ?, message_digest = ? WHERE dataset_id_key = ?'
  );
  $upd_sth->execute( $xml,            $compressed_xml,
                     $message_digest, $dataset_id_key );

  unlink "$xml_file.gz";
} ## end while ( my @row = $sel_sth...)

$sel_sth = $dbh->prepare(
'SELECT template, compressed_xml FROM meta_template__xml__dm INNER JOIN meta_template__template__main USING (template);'
);
$sel_sth->execute();
while ( my @row = $sel_sth->fetchrow_array() ) {
  my $template = $row[0];
  ...
  my $upd_sth = $dbh->prepare(
'UPDATE meta_template__xml__dm SET compressed_xml = ? WHERE template = ?'
  );
...
sub file_to_bytes {
  ...
  my $bytes = do { local $/; <$file> }
    or die "Could not read $file_name into memory";
  $bytes;
}
