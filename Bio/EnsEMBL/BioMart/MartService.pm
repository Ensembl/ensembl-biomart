...
sub new {
	...
	if ( !defined $self->{url} ) {
		if ( defined $self->{server} ) {
			$self->{url} =
			  sprintf( 'http://%s/biomart/martservice', $self->{server} );
		} else {
...
sub get_datasets {
	...
	my $response =
	  $self->do_get( { type => 'datasets', mart => $mart->name() } );
	my $datasets = [];
	open my $fh, '<', \$response or throw "Could not break ";
	while (<$fh>) {
		chomp;
		my ( $name, $des, $ass, $template ) =
		  ( split( '\t', $_ ) )[ 1, 2, 4, 7 ];
		if ( defined $name ) {
			push @$datasets,
...
sub do_post {
	...
	my $request =
	  HTTP::Request->new( "POST", $self->{url}, HTTP::Headers->new(),
						  $post_str );
